...
介入を実施したら **Claude に報告**して記録してもらう運用:

1. **介入を実施**（広告スロット変更、新規動画公開、戦略決定 等）
2. **Claude にメッセージで報告**
   - 自然文で OK。例:
     - 「動画 `abc123def` のスロットを 5 → 3 に減らした」
     - 「今月公開する動画から V2 タイトル戦略を採用する」
     - 「アンジュ卒業発表があった」
3. **Claude が必要情報を聞き返し → CSV に追記してコミット**
   - 不足情報（hypothesis, eval_after_days 等）はその場で確認
   - 既知の文脈（V2 戦略の hypothesis 等）は Claude が補完
4. **評価期日が来たら通知**
   - `analysis/intervention_log.py` を定期実行 → 期日 due の介入を抽出
   - Claude が before/after 分析を提案・実行
...
[介入報告]
日付: 2026-05-01
対象: bv2iwq17LVY（または channel）
種別: ad_slot
変更: スロット数 5 → 3、配置を avg_watch 60% 中心に
仮説: RPM +5-10%、CTR 維持
評価日数: 60 日
備考: 30+ 分動画のテスト第 1 弾
```

ただし**自然文の方が簡単**なら、Claude が必要事項を聞きながら CSV 化する。
**「介入したら即報告」が最重要**で、フォーマットは二の次。

## 運用ルール

### 介入の記録タイミング

**介入実施直後**に Claude に報告。記憶に頼らない。
